...
/// Awake 时委托 RoleViewFactory 实例化当前角色并托管其生命周期，完成后销毁自身。
/// </summary>
public class PlayerCreator : MonoBehaviour, IController
{
    ...
    private void Awake()
    {
        var viewFactory = this.GetUtility<IRoleViewFactory>();

        var instance = viewFactory.SpawnCurrentRole(transform.position, transform.rotation);
        if (instance == null)
        {
            Destroy(gameObject);
            return;
