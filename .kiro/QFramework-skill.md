...
## 创建System、Model、Utility
- 创建System、Model、Utility时，要注册到Architecture当中
- After creating any new ISystem implementation, add `this.RegisterSystem<IYourSystem>(new YourSystem());` in TianArchitecture.cs
- After creating any new IModel implementation, add `this.RegisterModel(new YourModel());` in TianArchitecture.cs  
- After creating any new IUtility implementation, add `this.RegisterUtility<IYourUtility>(new YourUtility());` in TianArchitecture.cs
- Do this in the same response where you create the System/Model/Utility, not as a separate step
